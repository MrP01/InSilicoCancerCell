#![allow(non_upper_case_globals)]

pub const dt: f64 = 0.004; // timestep
pub const STEPS_PER_MEASUREMENT: usize = 1;
pub const EvK: f64 = -77.4e-3; // reversal potential K

// pub const EvCa: f64 = 95.6e-3; // reversal potential Ca
// pub const EvCl: f64 = -7.9e-3; // reversal potential Cl
pub const Ca_i: f64 = 0.0647e-6; // initial calcium concentration
pub const F: f64 = 96485.3329; // As/mol
pub const R: f64 = 8.3144598; // kgm^2/s^2molK
pub const T: f64 = 273.0; // K, TODO: 0°C? melting point?
