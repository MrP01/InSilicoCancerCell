...
\begin{document}
  ...
  % \title{Optimization of an electrophysiological cancer cell model for a live simulation dashboard}
  ...
  The unconstrained least-squares problem could be solved very efficiently using a QR-decomposition of the current basis $R = Q_b R_b$, its solution would be $\vec{N}_{opt} = \lfloor R_b\inv Q_b^* \vec{I}_{\rm meas} \rfloor \in \Z^M$.

  \subsection{Formulation as Quadratic Problem}
  ...
  where we set $A := -\mathds{1}$ and $\vec{b} := \vec{0}$.
  ...
  \begin{table}
    ...
    \begin{tabular}{ll}
      ...
      Non-Negative Least Squares  & 1                 \\
      Quadratic Program           & 1                 \\
      Gradient Descent + Penalty  & 1                 \\
      LBFGS + Penalty             & 1                 \\
    \end{tabular}
